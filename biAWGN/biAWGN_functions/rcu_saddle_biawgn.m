...
for ii = 1:length(R_vec)
    R = R_vec(ii);
    rho_current(ii) = interp1(E0prime_vec,rho_vec,R,'linear',NaN);
    rho_vec_in1 = -0.9 ; 
    rho_vec_in2 = 10 ; 
    while isnan(rho_current(ii))
       if(E0prime_vec(1) < R)
          rho_vec_in1 = rho_vec_in1 - 10.9 ; 
          rho_vec_in2 = rho_vec_in2 - 10.9 ; 
       else
          rho_vec_in1 = rho_vec_in1 + 10.9 ; 
          rho_vec_in2 = rho_vec_in2 + 10.9 ; 
       end
       rho_current(ii) = Find_rho(R,rho_vec_in1,rho_vec_in2,snr,Zmax) ; 
    end
    if (R>Capacity)
...
function rho_current = Find_rho(R,rho_vec_in1,rho_vec_in2,snr,Zmax) 
   rho_vec = linspace(rho_vec_in1,rho_vec_in2,500);
   s_vec=1./(1+rho_vec);
   E0prime_vec = nan(size(rho_vec));
   for ii=1:length(rho_vec)
       E0prime_vec(ii) = E0prime(snr,rho_vec(ii),s_vec(ii),Zmax);
   ...
   rho_vec(rem_idx) = [];
   E0prime_vec = E0prime_vec + sort(1e-10.*randn(size(E0prime_vec)),'descend') ; 
   rho_current = interp1(E0prime_vec,rho_vec,R,'linear',NaN);
end
